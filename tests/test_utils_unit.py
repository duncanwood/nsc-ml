...
def test_strip_and_synth_objid_roundtrip():
    ...
    sid = swellar.synth_objid('163053_2831', params)
    assert sid.startswith('163053_2831_ml_')
    assert swellar.strip_objid(sid) == '163053_2831'


def test_convert_to_range_index():
    out = swellar.convert_to_range_index(np.array([3, 4, 5, 6]))
    assert isinstance(out, pd.RangeIndex)
    assert list(out) == [3, 4, 5, 6]
    gapped = np.array([3, 4, 7])
    assert list(swellar.convert_to_range_index(gapped)) == [3, 4, 7]  # unchanged


def test_get_default_args_and_common_params():
    defaults = swellar.get_default_args(swellar.find_persistent_excursions)
    assert defaults['z_threshold'] == 3
    assert defaults['timescale'] == 5
    common = swellar.common_params(swellar.find_persistent_excursions,
                                 {'z_threshold': 9, 'bogus': 1})
    assert common == {'z_threshold': 9}


def test_default_args_of_functions_union():
    params = swellar.default_args_of_functions(
        [swellar.find_persistent_excursions, swellar.fit_excursions])
...
def test_make_instrument_prefixes():
    ...
    out = swellar.make_instrument(df)
...
def test_extend_lc_window():
    ...
    ext = swellar.extend_lc(df, region, context_size=100)
...
def test_float_cols_to_double():
    ...
    out = swellar.float_cols_to_double(df)
...
def test_add_microlensing_event_darkens_and_relabels():
    ...
    out = swellar.add_microlensing_event(df, impact_parameter=0.05,
                                       crossing_time=20.0, peak_time=100.0)
    assert (out['objectid'] == swellar.synth_objid('src', {
...
def test_well_sampled_region_detects_dense_cluster():
    ...
    regions = swellar.well_sampled_region(df, interval=50, maxrevisit=10, seqlen=5)
...
def test_well_sampled_region_rejects_short_run():
    ...
    assert swellar.well_sampled_region(df, interval=50, maxrevisit=10, seqlen=5) == []


def test_compute_file_map(working, real_path, tmp_path):
    objfilemap, fileenum = swellar.compute_file_map([working['mini_lc'], real_path])
...
def test_reduce_and_nondetections():
    ...
    assert set(swellar.reduce_excursions(exc)) == {'a', 'c'}
    assert swellar.get_nondetections(exc) == ['b']
...
def test_split_real_synth_df_uses_ml_token():
    ...
    rdf, sdf = swellar.split_real_synth_df(df)
...
def test_consolidate_raises_on_metadata_mismatch(tmp_path):
    ...
    with pytest.raises(ValueError):
        swellar.consolidate_search_files_for_excursions([str(f1), str(f2)])
