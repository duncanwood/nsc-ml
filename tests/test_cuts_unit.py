...
def test_cut_pcov_boundary_is_strict():
    ...
    kept = nscml.cut_pcov(df)  # default cond_lim = 1e5
...
def test_cut_high_points_low_p_keeps_low_count_or_high_p():
    ...
    kept = nscml.cut_high_points_low_p(df, npoints=10, pval=0.05)
...
def test_cut_high_points_inout_low_p():
    ...
    kept = nscml.cut_high_points_inout_low_p(df, npoints=10, pval=0.05)
...
def test_cuts_compose_and_preserve_columns():
    ...
    out = nscml.cut_crossing_time(nscml.cut_pcov(nscml.cut_by_pval(df, 0.05)), 1, 100)
