...
def test_sparse_wma_constant_signal():
    ...
    y = np.full(12, 0.7)
    w = np.full(12, 2500.0)
    wma, wme, wms = nscml.sparse_gaussian_wma(y, t, w, timescale=TS, nclip=10)
...
def test_sparse_equals_dense_when_unclipped():
    ...
    s_wma, s_err, s_scat = nscml.sparse_gaussian_wma(y, t, w, timescale=TS, nclip=100)
    d_wma, d_err, d_scat = nscml.compute_weighted_moving_average(y, t, e, timescale=TS)
...
def test_nclip_actually_clips():
    ...
    wide = nscml.sparse_gaussian_wma(y, t, w, timescale=TS, nclip=100)[0]
    tight = nscml.sparse_gaussian_wma(y, t, w, timescale=TS, nclip=1)[0]
...
def test_dense_gaussian_matches_default_window():
    ...
    a = nscml.weighted_moving_average_gaussian(y, t, e, timescale=TS)
    b = nscml.compute_weighted_moving_average(y, t, e, timescale=TS)
    for x, z in zip(a, b):
...
def test_dispatcher_matches_backends():
    ki = fb.kernel_inputs()
    y, t, e, w = ki['y'], ki['t'], ki['errs'], ki['weights']
    sp = nscml.weighted_moving_average(y, t, e, sparse=True, timescale=TS, nclip=10)
    de = nscml.weighted_moving_average(y, t, e, sparse=False, timescale=TS)
    np.testing.assert_array_equal(sp[0], nscml.sparse_gaussian_wma(y, t, w, timescale=TS, nclip=10)[0])
    np.testing.assert_array_equal(de[0], nscml.compute_weighted_moving_average(y, t, e, timescale=TS)[0])


def test_weighted_avg_and_std_scale_invariance():
    v = np.array([1.0, 2.0, 3.0, 10.0])
    w = np.array([1.0, 1.0, 2.0, 0.5])
    a0, s0 = nscml.weighted_avg_and_std(v, w)
    a1, s1 = nscml.weighted_avg_and_std(v, 1000.0 * w)  # scaling weights is a no-op
...
def test_gaussian_window_shape():
    assert nscml.gaussian_window(0.0, 2.0) == pytest.approx(1.0)
    # symmetric and equal to the analytic gaussian
    for dt in (1.0, -1.0, 3.5):
        assert nscml.gaussian_window(dt, 2.0) == pytest.approx(np.exp(-(dt / 2.0) ** 2 / 2))
    assert nscml.gaussian_window(1.0, 2.0) == nscml.gaussian_window(-1.0, 2.0)


def test_clipped_window_truncates_beyond_nclip():
    ts, nclip = 2.0, 5
    assert nscml.clipped_gaussian_window(0.0, ts, nclip) == pytest.approx(1.0)
    # just inside the clip radius: equals the gaussian; just outside: exactly 0
    assert nscml.clipped_gaussian_window(9.9, ts, nclip) == pytest.approx(np.exp(-(9.9 / ts) ** 2 / 2))
    assert nscml.clipped_gaussian_window(10.1, ts, nclip) == 0.0
    assert nscml.clipped_gaussian_window(-10.1, ts, nclip) == 0.0
...
def test_amplification_physics():
    ...
    amp = nscml.microlensing_amplification(t, u0, tE, t0)
    ...
    amp_close = nscml.microlensing_amplification(np.array([t0]), 0.05, tE, t0)[0]
...
def test_blending_factor():
    ...
    full = nscml.microlensing_amplification(t, u0, tE, t0)
    blended = nscml.microlensing_amplification(t, u0, tE, t0, f)
    np.testing.assert_allclose(blended, full * f + (1 - f), atol=1e-12)


def test_amp_to_mag_and_ml_f():
    assert nscml.amp_to_mag(1.0) == pytest.approx(0.0)
    assert nscml.amp_to_mag(10.0) == pytest.approx(-2.5)
    t = np.linspace(90, 110, 9)
    u0, tE, t0 = 0.3, 12.0, 100.0
    np.testing.assert_array_equal(
        nscml.ml_f(t, u0, tE, t0),
        nscml.amp_to_mag(nscml.microlensing_amplification(t, u0, tE, t0)))
...
def test_ml_jac_matches_finite_difference():
    ...
    analytic = nscml.ml_jac(t, *p)
    steps = np.array([1e-6, 1e-4, 1e-4])
    for j in range(3):
        pp, pm = p.copy(), p.copy()
        pp[j] += steps[j]
        pm[j] -= steps[j]
        num = (nscml.ml_f(t, *pp) - nscml.ml_f(t, *pm)) / (2 * steps[j])
...
def test_ks_weighted_matches_scipy_statistic():
    ...
    k = fb.ks_inputs()
    d, _ = nscml.ks_weighted(k['a'], k['b'], np.ones_like(k['a']), np.ones_like(k['b']))
...
def test_ks_weighted_weight_sensitivity():
    ...
    d_skew, _ = nscml.ks_weighted(a, b, np.array([10.0, 1.0, 1.0, 1.0]), np.ones(3))
...
def test_reject_outliers_flags_extreme_point():
    ...
    keep = nscml.reject_outliers_args(data, m=3.0)
    assert keep[-1] == False
    assert keep[:-1].all()
    np.testing.assert_array_equal(nscml.reject_outliers(data, 3.0), data[keep])
...
def test_reject_outliers_zero_mad_keeps_all():
    ...
    data = np.full(6, 3.0)
    assert nscml.reject_outliers_args(data, 3.0).all()
...
def test_reject_low_error_outliers_uses_errors():
    ...
    assert nscml.reject_low_error_outliers_args(data, small, m=3)[-1] == False
    assert nscml.reject_low_error_outliers_args(data, big, m=3)[-1] == True
