...
def injected_lc(n=80, seed=0, u0=0.05, tE=20.0, t0=100.0, span=200.0):
    ...
    dm = rng.normal(0, 0.01, n) + nscml.amp_to_mag(
        nscml.microlensing_amplification(t, u0, tE, t0))
    return make_lc(t, dm)
...
def test_too_short_returns_no_regions(n):
    ...
    assert nscml.find_persistent_excursions(make_lc(t, np.zeros(n))) == []


def test_flat_curve_no_false_positive():
    t = np.linspace(0, 200, 60)
    assert nscml.find_persistent_excursions(make_lc(t, np.zeros(60))) == []
...
def test_injected_event_is_detected():
    regions = nscml.find_persistent_excursions(injected_lc())
...
def test_n_measured_gate_rejects_when_too_high():
    lc = injected_lc()
    assert nscml.find_persistent_excursions(lc, n_measured=1000) == []


def test_duration_gate_rejects_when_too_high():
    lc = injected_lc()
    assert nscml.find_persistent_excursions(lc, duration=1e6) == []


def test_z_threshold_controls_sensitivity():
    lc = injected_lc(u0=0.2)  # shallower event
    loose = nscml.find_persistent_excursions(lc, z_threshold=2)
    strict = nscml.find_persistent_excursions(lc, z_threshold=50)
...
def test_restrict_to_indices_masks_outside_window():
    ...
    full = nscml.find_persistent_excursions(lc)
    assert len(full) >= 1
    ...
    kept = nscml.find_persistent_excursions(lc, restrict_to_indices=event_idx)
    assert len(kept) >= 1
    complement = lc.index.difference(event_idx).to_numpy()
    assert nscml.find_persistent_excursions(lc, restrict_to_indices=complement) == []
...
def test_detection_is_band_agnostic():
    ...
    r_one = nscml.find_persistent_excursions(one)
    r_multi = nscml.find_persistent_excursions(multi)
...
def test_temper_errors_suppresses_detection():
    ...
    base = nscml.find_persistent_excursions(lc, temper_errors=None)
    tempered = nscml.find_persistent_excursions(lc, temper_errors=5)
...
def test_cut_outliers_path_runs():
    ...
    lc = injected_lc()
    regions = nscml.find_persistent_excursions(lc, cut_outliers=True)
